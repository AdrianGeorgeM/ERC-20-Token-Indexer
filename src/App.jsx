...
function App() {
	...
	function prettyBalance(numberAsString, index, decimalPlaces) {
		const str = Utils.formatUnits(
			numberAsString,
			tokenDataObjects[index] ? tokenDataObjects[index].decimals : 18
		);
						...
						>
							<HStack mt={2} spacing={{ base: 0, md: 1, lg: 3 }}>
								...
								<Input
									...
									id='addressInput'
									color='black'
									w={{ base: '54vw', md: '50vw', lg: '50vw' }}
									fontSize={{ base: '14px', md: 'md', lg: 'xl' }}
									textAlign='center'
									p={4}
									bgColor='white'
									value={userAddress}
					...
					<TabPanel className='nft'>
						...
						>
							...
							<Button
								...
								mb={20}
								fontSize={20}
								onClick={getNftsOwned}
							>
								Check NFTs owned!
							</Button>

							{isLoading && (
								<Image src='/Ethereum logo revolving.gif' borderRadius={30} />
							)}
							...
							{hasQueried ? (
								<>
									...
									<SimpleGrid w={'90vw'} columns={3} spacing={12} id='nftResults'>
										{nftResults.map((e, i) => {
											if (e.title.length == 0 && e.media.length == 0) {
												return;
											}
											...
											return (
												...
												>
													<Box>
														<Link href={e.tokenUri.gateway} target='_blank'>
															{e.title}
														</Link>
													</Box>
													<Link href={e.media[0].gateway} target='_blank'>
														<Image src={e.media[0].gateway} />
													</Link>
													<Box fontSize='xs'>Balance: {e.balance}</Box>
													<Box>
														Collection:{' '}
														<Link href={e.contract.openSea.externalUrl} target='_blank'>
															{e.contract.openSea.collectionName}
														</Link>
													</Box>
													<Box>
														{e.contract.openSea.floorPrice > 0 &&
															'Floor: ' + e.contract.openSea.floorPrice + ' ETH'}
													</Box>
												</Flex>
											);
										})}
